/*
  ...
  Header for emonPi Continuous Monitoring - radio using JeeLib RFM69 "Native"
  format
  ...
  V1.0.0   10/7/2021 Derived from emonLibCM examples and original emonPi sketch,
  that being derived from
            https://github.com/openenergymonitor/emonpi/blob/master/Atmega328/emonPi_RFM69CW_RF12Demo_DiscreteSampling
            and emonLibCM example sketches, with config input based on emonTx V3
  sketches.
...
const PROGMEM char helpText1[] = "|\n"
                                 ...
                                 "| b<n>\t\t- set r.f. band n = 4 > 433MHz, 8 > 868MHz, 9 > 915MHz (may "
                                 "require hardware change)\n"
                                 "| p<nn>\t\t- set r.f. power. nn (0 - 31) = -18 dBm to +13 dBm. Default: "
                                 "25 (+7 dBm)\n"
                                 ...
                                 "|\t\t\tx = 0 > all OFF, x = 1 > count 1 ON, x = 2 > count 2 ON, <yy> = "
                                 "pulse minimum period (ms) (y is not needed when x = 0)\n"
                                 "| t0 <y>\t- temperature:\n"
                                 ...
                                 "| T<ccc>\\n\t- transmit a string.\n"
                                 "| w<n>\t\t- n = 0 > radio OFF, n = 1 for Receive ON, n = 2 for Transmit "
                                 "ON, n = 3 for both ON\n"
...
static void load_config(void) { eepromRead(eepromSig, (byte *)&EEProm); }

static void list_calibration(void) {

  Serial.println(F("|Settings"));
  Serial.print(F("|Radio: "));
  Serial.println(EEProm.rfOn);
  Serial.print(F("|RF Band: "));
  if (EEProm.RF_freq == RFM_433MHZ)
    Serial.println(F("433MHz"));
  if (EEProm.RF_freq == RFM_868MHZ)
    Serial.println(F("868MHz"));
  if (EEProm.RF_freq == RFM_915MHZ)
    Serial.println(F("915MHz"));
  Serial.print(F("|Power: "));
  Serial.print(EEProm.rfPower - 18);
  Serial.println(F(" dBm"));
  Serial.print(F("|Group: "));
  Serial.println(EEProm.networkGroup);
  Serial.print(F("|Node ID: "));
  Serial.println(EEProm.nodeID);
  Serial.println(F("|Calibration"));
  Serial.print(F("|assumedV: "));
  Serial.println(EEProm.assumedVrms);
  Serial.print(F("|AC Detected: "));
  Serial.println(EmonLibCM_acPresent() ? "Yes" : "No");
  Serial.print(F("|freq: "));
  Serial.println(EEProm.lineFreq);
  Serial.print(F("|vCal: "));
  Serial.println(EEProm.vCal);
  Serial.print(F("|i1Cal: "));
  Serial.print(EEProm.i1Cal);
  Serial.print(F(", Lead: "));
  Serial.println(EEProm.i1Lead);
  Serial.print(F("|i2Cal: "));
  Serial.print(EEProm.i2Cal);
  Serial.print(F(", Lead: "));
  Serial.println(EEProm.i2Lead);
  Serial.print(F("|datalog: "));
  Serial.println(EEProm.period);
  Serial.print(F("|pulses1: "));
  Serial.print(EEProm.pulse_enable);
  Serial.print(F(", period: "));
  Serial.println(EEProm.pulse_period);
  Serial.print(F("|pulses2: "));
  Serial.print(EEProm.pulse2_enable);
  Serial.print(F(", period: "));
  Serial.println(EEProm.pulse2_period);
  Serial.print(F("|temp_enable: "));
  Serial.print(EEProm.temp_enable);
  Serial.print(F(", sensors found: "));
  Serial.println(EmonLibCM_getTemperatureSensorCount());
  if (verbose)
...
static void report_calibration(void) {
  ...
  Serial.print(F(" ac"));
  Serial.print(EmonLibCM_acPresent());
  Serial.print(F(" f"));
  Serial.print(EEProm.lineFreq);
  Serial.print(F(" k0 "));
  Serial.print(EEProm.vCal);
  Serial.print(F(" 0.00"));
  Serial.print(F(" k1 "));
  Serial.print(EEProm.i1Cal);
  Serial.print(F(" "));
  Serial.print(EEProm.i1Lead);
  Serial.print(F(" k2 "));
  Serial.print(EEProm.i2Cal);
  Serial.print(F(" "));
  Serial.print(EEProm.i2Lead);
  Serial.print(F(" d"));
  Serial.print(EEProm.period);
  Serial.print(F(" m"));
  Serial.print(EEProm.pulse_enable);
  Serial.print(F(" "));
  ...
  Serial.print(EEProm.pulse2_enable);
  Serial.print(F(" "));
  Serial.print(EEProm.pulse2_period);
  Serial.print(F(" t"));
  Serial.print(EEProm.temp_enable);
  Serial.print(F(" "));
  Serial.println(EmonLibCM_getTemperatureSensorCount());
...
void getCalibration(void) {
  /*
     Reads calibration information (if available) from the serial port at
     runtime. Data is expected generally in the format
     ...
     where:
      ...
      see the user instruction above, the comments below or the separate
    ...
    if (!lockout(c))
      ...
      case 'c':
        ...
        */
        ...
        switch (k1) {
        case 1:
          EEProm.showCurrents = true;
          break;
        default:
          EEProm.showCurrents = false;
        }
        break;
      ...
      case 'k':
        /*  Format expected: k[x] [y] [z]
          ...
          2 = ct2 calibration, etc [y] = a floating point number for the
          voltage/current calibration constant [z] = a floating point number for
          the phase calibration for this c.t. (z is not needed, or ignored if
          supplied, when x = 0)

          e.g. k0 256.8
               k1 90.9 1.7

          If power factor is not displayed, it is impossible to calibrate for
          phase errors, and the standard value of phase calibration MUST BE SENT
          when a current calibration is changed.
        */
        k1 = Serial.parseInt();
        k2 = Serial.parseFloat();
      ...
      case 'm':
        /*  Format expected: m[x] [y]
           ...
           where:
            [x] = a single numeral: 0 = pulses OFF, 1 = pulses 1 ON, 2 = pulses
           2 ON [y] = an integer for the pulse min period in ms - ignored when
           x=0
        */
        k1 = Serial.parseInt();
        k2 = Serial.parseInt();
        ...
        switch (k1) {
        case 0:
          EmonLibCM_setPulseEnable(0, false);
        ...
        case 1:
          EmonLibCM_setPulseMinPeriod(0, k2);
        ...
        case 2:
          EmonLibCM_setPulseMinPeriod(1, k2);
      ...
      case 'p': // set RF power level
        ...
        if (verbose) {
          Serial.print(F("|p: "));
          Serial.print((EEProm.rfPower & 0x1F) - 18);
          Serial.println(F(" dBm"));
        }
        rfChanged = true;
        break;
      ...
      case 'T': {
        ...
        if (outmsgLength > 1) { // outmsgLength must be >1, byte1 = destid, byteN=payload
          outmsgLength--;       // we increased it above artificially when dealing with txDestId
          Serial.print(F("Queueing  "));
          Serial.print(outmsgLength);
          Serial.print(F(" bytes to send to node "));
          Serial.println(txDestId);
        } else {
          outmsgLength = 0; // In case we only receive a single byte from serial, we would have destId with no payload
        }
      ...
      case 'V': // Verbose mode
        ...
        */
        ...
        Serial.println(verbose ? F("on") : F("off"));
        break;
...
void set_temperatures(void) {
  /*  Format expected: t[x] [y] [y] ...
    ...
    where:
     [x] = 0  [y] = single numeral: 0 = temperature measurement OFF, 1 =
    temperature measurement ON, 2 = search [x] = a single numeral > 0: the
    position of the sensor in the list (1-based) [y] = 8 hexadecimal bytes
    representing the sensor's address e.g. t2 28 81 43 31 07 00 00 D9
  */
