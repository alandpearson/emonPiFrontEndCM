...
const byte firmware_version[3] = {1, 0, 2};
/*
...
const byte TEMPERATURE_PRECISION = 11; // 9 (93.8ms), 10 (187.5ms), 11 (375ms) or 12 (750ms) bits equal to resolution of
...
struct {
  ...
  byte networkGroup =
      210;             // wireless network group, must be the same as emonBase / emonPi and emonGLCD. OEM default is 210
...
void setup() {
  ...
  EmonLibCM_SetADC_IChannel(1, EEProm.i1Cal,
                            EEProm.i1Lead); // ADC Input channel, current calibration, phase calibration
  ...
  EmonLibCM_setTemperatureResolution(
      TEMPERATURE_PRECISION); // Resolution in bits, allowed values 9 - 12. 11-bit resolution, reads to 0.125 degC
...
#ifdef EEWL_DEBUG
  Serial.print(F("End of mem="));
  Serial.print(E2END);
  Serial.print(F(" Avail mem="));
  Serial.print((E2END >> 2) * 3);
  Serial.print(F(" Start addr="));
  Serial.print(E2END - (((E2END >> 2) * 3) / (sizeof(mem) + 1)) * (sizeof(mem) + 1));
  Serial.print(F(" Num blocks="));
  Serial.println(((E2END >> 2) * 3) / 21);
  EVmem.dump_buffer();
#endif
...
/**************************************************************************************************************************************************
 *
 * LOOP         Read the pushbutton, poll the radio for incoming data, the serial input for calibration & emonLibCM for
 *energy readings
 *
 ***************************************************************************************************************************************************/
...
void loop() {
  ...
  if ((EEProm.rfOn & RFTX) && outmsgLength) { // if command 'outmsg' is waiting to be sent then let's send it
    ...
    rf.send(txDestId, (void *)outmsg,
            outmsgLength); //  void RF69<SPI>::send (uint8_t header, const void* ptr, int len) {
    Serial.print("Sent ");
    Serial.print((word)outmsgLength);
    Serial.print(" bytes to node ");
  ...
  if (EmonLibCM_Ready()) {
    ...
    if (EEProm.showCurrents) {
      ...
      Serial.print(F("|Vrms:"));
      Serial.print(EmonLibCM_getVrms());

      Serial.print(F(", I1:"));
      Serial.print(EmonLibCM_getIrms(0));
      Serial.print(F(", pf1:"));
      Serial.print(EmonLibCM_getPF(0), 4);

      Serial.print(F(", I2:"));
      Serial.print(EmonLibCM_getIrms(1));
      Serial.print(F(", pf2:"));
      Serial.print(EmonLibCM_getPF(1), 4);

      Serial.print(F(", f:"));
      Serial.print(EmonLibCM_getLineFrequency());
      Serial.println();

      // lcd_print_currents(current_lcd_i2c_addr, EmonLibCM_getIrms(0), EmonLibCM_getIrms(1), EmonLibCM_getPF(0),
      // EmonLibCM_getPF(1));
    }
    delay(50);
  ...
  /*
    ...
    if (backlight && (millis() - backlightOn) > BACKLIGHT_TIMEOUT)
    {
      ...
      lcd.noBacklight();                                                   // It can get left on if this is restarted
    without the Pi rebooting backlight = false;
    }
...
int i2c_LCD_Detect(int i2c_lcd_address[]) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (error == 0) {
      Serial.print(F("LCD found i2c 0x"));
      Serial.println(i2c_lcd_address[i], HEX);
...
void Startup_to_LCD(int current_lcd_i2c_addr) // Print to LCD
{
  ...
  lcd.print(EmonLibCM_getTemperatureSensorCount());
  lcd.setCursor(0, 1);
